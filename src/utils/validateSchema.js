exports.validate = function(schema) {
  const validateParamContent = (content, element) => {
    ...
    if (
      ...
      throw new Error(
        `Wrong schema: Parameter: ${element} has wrong type value: ${content.type}`
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Wrong schema: Parameter: ${element} has wrong format value: ${
          content.format
        }`
      );
    }
    ...
    if (content.type === "array") {
      if (!content.items) {
        throw new Error(
          `Wrong schema: Parameters section is not defined for array.`
        );
      }
